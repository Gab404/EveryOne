...
def api_models_delete(payload: ModelDeleteRequest):
    """Supprime physiquement un modèle GGUF du disque dur."""
    models = discover_local_gguf_models()
    
    for model in models:
        if model.key == payload.key:
            try:
                # Vérifie si le fichier existe et le supprime
                if model.path.exists():
                    model.path.unlink()
                    return {"status": "ok", "message": f"Modèle {model.path.name} supprimé avec succès."}
            except Exception as e:
                raise HTTPException(status_code=500, detail=f"Impossible de supprimer le fichier: {str(e)}")
                
    raise HTTPException(status_code=404, detail="Modèle introuvable sur le disque.")
...
SIMULATED_JOBS_PROGRESS = {}

@app.get("/api/finetune/status/{job_id}")
async def api_finetune_status(job_id: str):
    """Vérifie le statut du job et renvoie un pourcentage de progression"""
    
    # Initialise le job à 0% s'il est nouveau
    if job_id not in SIMULATED_JOBS_PROGRESS:
        SIMULATED_JOBS_PROGRESS[job_id] = 0
        
    # Fait avancer le pourcentage aléatoirement entre 8% et 22% à chaque appel
    SIMULATED_JOBS_PROGRESS[job_id] += random.randint(8, 22)
    
    # Cap à 100%
    if SIMULATED_JOBS_PROGRESS[job_id] >= 100:
        SIMULATED_JOBS_PROGRESS[job_id] = 100
        return {
            "job_id": job_id, 
            "status": "completed", 
            "progress": 100
    ...
    return {
        "job_id": job_id, 
        "status": "running", 
        "progress": SIMULATED_JOBS_PROGRESS[job_id]
    }

@app.post("/api/finetune/cancel/{job_id}")
async def api_cancel_finetune(job_id: str):
    """Annule concrètement le job de fine-tuning."""
    
    # 1. Si tu utilises le vrai Modal plus tard, tu feras un appel ici :
    # async with httpx.AsyncClient() as client:
    #     await client.post(f"https://gab404--llama32-gguf.../cancel?job_id={job_id}")
    
    # 2. Pour notre simulation locale actuelle :
    if job_id in SIMULATED_JOBS_PROGRESS:
        del SIMULATED_JOBS_PROGRESS[job_id] # On supprime la trace du job
        
    return {"status": "ok", "message": f"Job {job_id} annulé avec succès."}